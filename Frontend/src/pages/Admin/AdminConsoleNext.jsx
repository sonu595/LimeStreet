            ...
            {activeTab === 'products' && (
              <div className="grid gap-6 xl:grid-cols-[minmax(0,1.08fr)_minmax(320px,0.92fr)]">
                ...
                <div className="rounded-[32px] border border-white/10 bg-zinc-950 p-5">
                  ...
                  <div className="mt-4 space-y-3">{products.map((product) => <div key={product.id} className="flex items-center gap-3 rounded-2xl border border-white/10 bg-black/30 p-3"><img src={product.imageUrls?.[0] || product.imageUrl} alt={product.name} className="h-16 w-14 rounded-xl object-cover" /><div className="min-w-0 flex-1"><p className="line-clamp-1 text-sm text-white">{product.name}</p><p className="mt-1 text-xs text-zinc-500">{product.sizes?.join(', ') || 'No sizes'} • {product.colors?.join(', ') || 'No colors'}</p></div><div className="flex gap-2"><button type="button" onClick={() => handleEdit(product)} className="rounded-full border border-white/10 px-3 py-2 text-xs text-zinc-300">Edit</button><button type="button" onClick={() => handleDelete(product.id)} className="rounded-full border border-red-500/20 px-3 py-2 text-xs text-red-300"><Trash2 size={14} /></button></div></div>)}</div>
                </div>
              </div>
            )}
